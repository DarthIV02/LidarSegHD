...
def get_tls_label_name(label_mapping):
    with open(label_mapping, 'r') as stream:
        tlsyaml = yaml.safe_load(stream)
    tls_label_name = dict()
    for i in sorted(list(tlsyaml['learning_map'].keys()))[::-1]:
       tls_label_name[tlsyaml['learning_map'][i]] = tlsyaml['labels'][i]

    return tls_label_name
