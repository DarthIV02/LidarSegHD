...
from dataloader.pc_dataset import get_tls_label_name
from builder import data_builder, model_builder, loss_builder
from config.config import load_config_data
from dataloader.dataset_tls import get_model_class, collate_fn_BEV
...
def main(args):
    ...
    print("Demo Label DIR", demo_label_dir)
    ...
    SemKITTI_label_name = get_tls_label_name(dataset_config["label_mapping"])
        ...
        for i_iter_demo, (_, demo_vox_label, demo_grid, demo_pt_labs, demo_pt_fea) in enumerate(
            ...
            for count, i_demo_grid in enumerate(demo_grid):
                try:
                   ...
                   print(len(demo_grid))
                   print(demo_grid[0][:,0])
                   inv_labels = predict_labels[count, demo_grid[count][:, 0], demo_grid[count][:, 1], demo_grid[count][:, 2]] 
                   print(inv_labels)
                   ...
                   print("save " + outputPath)
                    ...
                    print(f"KeyError: {e} occurred at count: {count}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-y', '--config_path', default='config/tls.yaml')
