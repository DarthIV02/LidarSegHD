...
class Network:
    ...
    def inference(self, inputs, is_training):
        ...
        # HD part goes here
        xyz, neigh_idx, sub_idx = self.input_augment(is_training, inputs['xyz'][0], inputs['neigh_idx'][0],
                                                         inputs['sub_idx'][0])
        print(xyz)
        print(neigh_idx)
        print(sub_idx)
    ...
    def encode_points(self, x, y, z, input=None, features = [True]*5):
        ...
        if features[1]:
            enc_points += self.z_emb(torch.tensor(z)).sign() + self.y_emb(torch.tensor(y)).sign() +self.x_emb(torch.tensor(x)).sign() # Height enc_points
        if features[2]:
    ...
    def encode_graph(self, id_point, point, points, distances=None, labels=None):
        ...
        if (distances == None):
            rad_array = [float(i) for i in self.opt.areas.split(",")]
            close_points_num, close_points_rad = self.get_close_points_euclidean(point[:3], points[:,:3], rad_array)
        ...
        if labels != None:
            classes_count = torch.bincount(labels[close_points_num])
        
        #If we vecotrize based on the current point
        vectors_closest_points = points[:,:3][close_points_num] - point[:3]
        #vectors_closest_points = (points[:,:3][close_points_num] + point[:3])/2
        ...
        features = [False]*5
        for i in self.opt.features.split(","):
            features[int(i)] = True
        enc_vectors = self.encode_points(vectors_closest_points[:, 0], vectors_closest_points[:, 1], vectors_closest_points[:, 2], points[:,:3][close_points_num], close_points_rad, features=features) # In case of intensity points[close_points][:, 3]
    ...
    def get_close_points_euclidean(self, point, points, rads_array): # TODO HD KNN? -> HD clustering by whose work has studied 
        ...
        all_data_tensor = []
        for r in rads_array:
            small_dist_i = (dist < r).nonzero()
            small_dist_i = torch.reshape(small_dist_i, (1, small_dist_i.shape[0]))[0]
            all_data_tensor.append(small_dist_i)
        return index_closest_points.indices[1:], all_data_tensor
