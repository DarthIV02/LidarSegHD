...
class SegmentHD:
    ...
    def __init__(self, d=1000, num_classes=5, num_closest_points=5, training_samples_per_class = 100, lr = 0.01) -> None:
        self.d = d
        ...
        self.x_emb = torchhd.embeddings.Projection(self.num_closest_points, d)
        self.y_emb = torchhd.embeddings.Projection(self.num_closest_points, d) 
        self.z_emb = torchhd.embeddings.Projection(self.num_closest_points, d)
        self.var = torchhd.random(3, d) # x, y, z, i 
        self.classes_hv = torch.zeros(classes, d) # alive, 1h, 10h, 100h, 1000h
    ...
    def encode_graph(self, id_point, point, points, distances=None):
        ...
        enc_vectors = model.encode_points(vectors_closest_points[:, 0], vectors_closest_points[:, 1], vectors_closest_points[:, 2]) # In case of intensity points[close_points][:, 3]
        ...
        for label in range(1, self.num_classes+1):
            ...
            if(classes_id[0].shape[0] != 0):
                ...
                for i, point in zip(training_samples, points[training_samples]): # id, xyz of points
                    ...
                    current_point_graph = model.encode_graph(i, point, points)
        ...
        for label in range(1, self.num_classes+1):
            ...
            if(classes_id[0].shape[0] != 0):
                ...
                for i, point in zip(training_samples, points[training_samples]):
                    ...
                    pred_class = torch.argmax(sim).item()
                    self.count_matrix[int(label-1), pred_class] += 1
        return

if __name__ == '__main__':
    d = 7500 # dimensions
    number_points = 500 # Number of closest points to consider for the graph
    classes = 5
    number_of_choices = 200
    epochs = 1
    lr = 0.01
    model = SegmentHD(d, classes, number_points, number_of_choices, lr)
    np.random.seed(0)
    torch.manual_seed(0)

    print("The hyperparameters are:")
    print(f"d = {d}")
    print(f"Number of close points = {number_points}")
    print(f"Number of points per class / file = {number_of_choices}")
    ...
    for e in range(epochs):
        for file in tqdm(os.listdir('Dataset_TLS/dense_dataset_semantic/sequences/00/velodyne/')):
            file = file[:-4]
        ...
        print(f"Epoch: {e}")
        print(model.classes_hv)
        sim = torchhd.cosine_similarity(model.classes_hv, model.classes_hv)
        print(sim)
